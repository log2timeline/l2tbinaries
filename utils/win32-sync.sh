...
then
  ...
  echo "";
  echo "  PATH: path of the directory containing the Windows 32-bit wheel files.";
  echo "";
...
do
  ...
  then
    ...
    EXISTING_FILE=`ls -1 win32/${PACKAGE_NAME}-*-${PYTHON_VERSION}-*-*.whl 2> /dev/null`;
...
# (cd win32 && sha256sum *win32*.whl >> SHA256SUMS)
